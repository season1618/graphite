...
`);
  ...
  return (
    <div id="app">
      ...
      <div id="flex"
        style={{ height: windowSize.height - 60 }}
        onMouseMove={
          (e) => {
      ...
      >
        <CodeEditor height={windowSize.height - 60} width={borderX} code={code} msg={msg} setCode={setCode} setMsg={setMsg} setTokens={setTokens}/>
        <div id="border"
          style={{left: borderX - width/2, width }}
          onMouseDown={() => setDragged(true)}
          onMouseUp={() => setDragged(false)}
        />
        <Canvas height={windowSize.height - 60} width={windowSize.width - borderX} setCode={setCode} tokens={tokens} prog={prog}/>
      </div>
    </div>
  );
}
